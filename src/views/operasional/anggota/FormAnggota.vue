<template>
      ...
      <div class="flex-col w-3/4">
        <div class="flex-row">
          <div class="flex-col mb-6">
            ...
            <input v-model="state.nama_lengkap" class="w-full border-2 border-primary-400 py-2 px-2 mr-4 rounded" placeholder="Contoh “Agung Bayu”">
          ...
          <div class="flex-col mb-6">
            ...
            <p class="text-sm font-medium text-grey-500 mb-2">Pastikan email dapat diakses</p>
            <input v-model="state.email" class="w-full border-2 border-primary-400 py-2 px-2 mr-4 rounded" placeholder="Contoh “agungbay@gmail.com”">
          ...
          <div class="flex-col mb-6">
            ...
            <p class="text-sm font-medium text-grey-500 mb-2">Username digunakan untuk masuk ke dalam aplikasi, pastikan mudah diingat</p>
            <input v-model="state.username" class="w-full border-2 border-primary-400 py-2 px-2 mr-4 rounded" placeholder="Contoh “agungBayu001”">
          ...
          <div class="flex-col mb-6">
            ...
            <input v-model="state.no_hp" class="w-full border-2 border-primary-400 py-2 px-2 mr-4 rounded" placeholder="Contoh “Contoh “081234567890”">
            ...
            <div class="flex">
                <div class="flex-col mr-6">
                    <div class="flex items-center">
                        <input type="radio" id="status" value="aktif" class="h-6 w-6 bg-gray-500 mr-4" v-model="state.status">
                        <label class="text-normal font-normal text-gray-500">Aktif</label>
                    </div>
                </div>
                <div class="flex-col">
                    <div class="flex items-center">
                        <input type="radio" id="status" value="nonaktif" class="h-6 w-6 bg-gray-500 mr-4" v-model="state.status">
    ...
    <div class="flex mx-16 mt-8 border-b-2 pb-8 border-dotted">
        ...
        <div class="flex-col w-3/4">
            <div class="flex-row">
               <div class="flex-col mb-6">
                    <p class="text-base font-normal text-base-500 mb-2">Kata Sandi</p>
                    <p class="text-sm font-medium text-grey-500 mb-2">Pastikan kata sandi hanya admin dan anggota yang tahu</p>
                    <div class="relative">
                        <input v-model="state.password" class="border border-primary-500 w-full mt-4 rounded py-3 px-4" :type="show ? 'text' : 'password'" placeholder="Tulis kata sandi disini....">
               ...
               <div class="flex-col mb-6">
                    <p class="text-base font-normal text-base-500 mb-2">Konfirmasi Kata Sandi</p>
                    <p class="text-sm font-medium text-grey-500 mb-2">Pastikan kata sandi sama dengan kolom diatas</p>
                    <div class="relative">
                        <input v-model="state.confirmPassword" class="border border-primary-500 w-full mt-4 rounded py-3 px-4" :type="show ? 'text' : 'password'" placeholder="Tulis ulang kata sandi disini....">
                        <div class="absolute inset-y-3 right-0 pr-3 pt-3 flex items-center text-sm leading-5">
                            <div v-if="!show" @click="show=!show">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-gray-500">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                    ...
                    <div v-if="v$.confirmPassword.$error" class="text-xs text-red-600">Konfirmasi kata sandi berbeda. Silakan ulang kembali.</div>
                </div>
            </div>
        </div>
    </div>
    <div class="flex justify-end mx-16 mt-8">
      <outlined-button class="mr-4 px-12" @click="$router.go(-1)">Batalkan</outlined-button>
      <primary-button class="px-12" @click="addNewAnggota">Simpan</primary-button>
    </div>
    </Operasional>
  </template>
  ...
  import { minLength, required, email, maxLength, sameAs, helpers, alphaNum } from '@vuelidate/validators'
      ...
      setup() {
        ...
        const rules = computed(() => {
            return {
                ...
                username: { required, minLength: minLength(4), maxLength: maxLength(11), alphaNum },
                password:{
                  required,
        ...
        const addNewAnggota = async () => {           
          v$.value.$touch()

          if (!v$.value.$invalid) {
            await userStore.addAnggota(    
              state.nama_lengkap,
              state.username,
              state.password,
              state.no_hp,
        ...
        return{state, v$, addNewAnggota, userStore}
      },
      data() {
        return {
            show: false
