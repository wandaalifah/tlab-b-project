<template>
  <Inventory>
    <div class="flex justify-between p-4 mb-4 mx-4">
      <h3 class="font-domine text-xl font-bold my-auto">Ubah Jenis Produk</h3>
      ...
      <div class="flex-col w-3/4">
        <div class="flex-row">
          <div class="flex-col mb-6">
            ...
            <input v-model="data.nama" class="w-full border-2 border-primary-400 py-2 px-2 mr-4 rounded" placeholder="Contoh “Emas Logam Mulia”">
          </div>
          <div class="flex-col">
            ...
            <input v-model="data.kode" class="w-full border-2 border-primary-400 py-2 px-2 mr-4 rounded" placeholder="Contoh “EML”">
          </div>
        </div>
      </div>
    </div>
    <div class="flex justify-end mx-16 mt-8">
      ...
      <primary-button class="px-12" @click="editJenis">Simpan</primary-button>
    </div>
  </Inventory>
</template>
...
export default {
    ...
    setup(props) {
      ...
      const data = ref([null])
      const jenis = ref([jenisStore.getJenisById(`${props.id}`).then(jenis => {
        data.value = jenis
      })])

      const editJenis = async () => {
        await jenisStore.updateJenis(
          data.value.ID,
          data.value.nama,
          data.value.kode
        )
        await router.push('/jenis')
      }

      return {data, jenis, editJenis}
    }
}
</script>
