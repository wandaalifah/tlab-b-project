<template>
    <Inventory>
        <div class="flex justify-between p-4 mb-4 mx-4">
            <h3 class="font-domine text-xl font-bold my-auto">Bentuk Produk</h3>
            <router-link :to="{name: 'AddBentuk'}">
                <add-button>Tambah Bentuk Produk</add-button>
            </router-link>
          </div>
          <hr>
          <!-- search bar -->
          <div class="flex flex-wrap p-4 mx-4 font-normal text-base text-grey-500">
            <div class="relative">
              <input v-model="searchQuery" class="w-[400px] border-2 border-primary-400 py-2 px-8 mr-4 rounded" placeholder="Cari bentuk produk atau kode produk">
            ...
            <div class="relative">
              <select v-model="sortDirection" class="text-grey-500 border-primary-400 border-2 rounded p-2">
                <option value="desc">Urutan berdasarkan bentuk produk terbaru</option>
                <option value="asc">Urutan berdasarkan bentuk produk terdahulu</option>
              </select>
            </div>
          </div>
          <!-- tabel data -->
          <div class="mx-8 mt-2 font-normal text-grey-900">
            ...
            <!-- table data -->
            <div v-for="bentuk in searchedResult" :key="bentuk">
              <div class="flex flex-row text-normal my-4">
                <p class="w-2/6 font-bold text-primary-800">{{ bentuk.nama }}</p>
                <p class="w-2/6">{{ bentuk.kode }}</p>
                <p class="w-2/6">{{ formatDate(bentuk.CreatedAt) }}</p>
                <router-link :to="{name: 'EditBentuk', params: {id: bentuk.ID}}">
...
export default {
  ...
  components: { Inventory, AddButton },
    setup() {
      ...
      const getBentuk = async () => {
        const response = await bentukStore.getBentuk()
